"""
================================================================================
butterfly_proxy_asia.py — Asifah Analytics
================================================================================
BUTTERFLY PROXY (Asia Backend)

Mirrors jawboning_proxy_wha.py architecture. The Asia backend's trackers (US,
Cuba, Mexico, Venezuela, Peru, Chile, etc.) call this proxy to fetch their
butterfly signal bundle. Proxy:
  1. Checks Asia local Redis for cached bundle
  2. If fresh (< 1h), returns it immediately
  3. If stale or missing, HTTP-fetches from ME backend's /api/butterfly/read
  4. Caches the fresh bundle to Asia local Redis
...
--------------------
    ...
    bundle = read_butterfly_signals_via_proxy(consumer_theater='india')
...
---------------------
    GET /api/asia/butterfly/<consumer_theater>     — proxy passthrough
    GET /api/asia/butterfly-debug                  — diagnostic
...
# ============================================================================
# REDIS HELPERS (local Asia Redis — for caching the proxy response)
# ============================================================================

def _cache_key(consumer_theater):
    return f"butterfly:asia:{consumer_theater}:cached"


def _redis_load(consumer_theater):
    """Load cached butterfly bundle from Asia Redis. Returns None if missing or stale."""
    ...
    try:
        ...
        print(f"[Butterfly Proxy Asia] Redis load error: {e}")
        return None


def _redis_save(consumer_theater, bundle):
    """Save butterfly bundle to Asia Redis with TTL."""
    if not (UPSTASH_REDIS_URL and UPSTASH_REDIS_TOKEN):
        return
    try:
        ...
        print(f"[Butterfly Proxy Asia] Redis save error: {e}")
...
def _fetch_from_me(consumer_theater):
    ...
    try:
        ...
        if not resp.ok:
            print(f"[Butterfly Proxy Asia] ME backend HTTP {resp.status_code} "
                  f"for {consumer_theater}")
            return None
        return resp.json()
    except requests.Timeout:
        print(f"[Butterfly Proxy Asia] Timeout calling ME backend "
              f"for {consumer_theater} (>{FETCH_TIMEOUT_SECONDS}s)")
        return None
    except Exception as e:
        print(f"[Butterfly Proxy Asia] ME backend error for {consumer_theater}: "
...
def read_butterfly_signals_via_proxy(consumer_theater, force=False):
    ...
    Args:
        consumer_theater: str — 'india', 'china', 'taiwan', 'japan', 'pakistan', etc.
    ...
    try:
        ...
        try:
            ...
            print(f"[Butterfly Proxy Asia] ⏱️ Hard timeout fetching {consumer_theater}")
            fresh = None
    except Exception as e:
        print(f"[Butterfly Proxy Asia] Fetch executor error: {e}")
        fresh = None
    finally:
...
def register_butterfly_proxy(app):
    """Register butterfly proxy endpoints on the Asia backend Flask app."""
    from flask import jsonify, request

    @app.route('/api/asia/butterfly/<consumer_theater>', methods=['GET', 'OPTIONS'])
    def api_asia_butterfly(consumer_theater):
    ...
    @app.route('/api/asia/butterfly-debug', methods=['GET'])
    def api_asia_butterfly_debug():
        return jsonify({
            'success':           True,
            ...
            'service':           'butterfly_proxy_asia',
            'version':           '1.0.0',
        })

    print("[Butterfly Proxy Asia] ✅ Endpoints registered:")
    print("  GET /api/asia/butterfly/<consumer_theater>")
    print("  GET /api/asia/butterfly-debug")
