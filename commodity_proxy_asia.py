"""
═══════════════════════════════════════════════════════════════════════
  ASIFAH ANALYTICS — ASIA BACKEND COMMODITY PROXY
  v1.0.0 (May 7 2026)
═══════════════════════════════════════════════════════════════════════

Thin proxy layer that fetches commodity-pressure data from the ME
backend (where commodity_tracker.py lives), caches it in Asia's
Upstash Redis with a 12-hour TTL, and exposes Asia-native endpoints
for stability page consumption.

ARCHITECTURE:
  Frontend (china-stability.html, japan-stability.html, taiwan-stability.html)
    └─→ Asia backend /api/asia/commodity/<target>
...
TARGETS SUPPORTED:
  ...
  Phase 1: china (already registered).
  Phase 2: japan, taiwan (to be added to ME COUNTRY_COMMODITY_EXPOSURE).
  When ME backend extends the dict, this proxy automatically supports
  the new countries (no Asia code change required).

ENDPOINTS REGISTERED:
  GET /api/asia/commodity/<target>            — single country, cached
  GET /api/asia/commodity/<target>?force=true — bypass Asia cache
  GET /api/asia/commodity-debug               — proxy status diagnostic
...
PATTERN PARITY:
  This module is a structural clone of commodity_proxy_europe.py — same
  cascade behavior, same diagnostic shape, same background refresh
  cadence. The only differences are namespace prefix ('asia:' vs 'europe:')
  and PROACTIVE_REFRESH_TARGETS list. Keeping these in lockstep means a
  bug fix in one is trivially portable to the other.
"""
...
def _redis_key(target):
    return f"asia:commodity:{target.lower()}"
...
def _load_from_redis(target):
    ...
    try:
        ...
        print(f"[Commodity Proxy Asia] Redis load error for {target}: {e}")
    return None
...
def _save_to_redis(target, payload):
    ...
    try:
        ...
        if resp.status_code == 200:
            print(f"[Commodity Proxy Asia] ✅ Cached {target} in Asia Redis")
            return True
        else:
            print(f"[Commodity Proxy Asia] Redis save HTTP {resp.status_code} for {target}")
    except Exception as e:
        print(f"[Commodity Proxy Asia] Redis save error for {target}: {e}")
    return False
...
def _fetch_from_me_backend(target):
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f"[Commodity Proxy Asia] ME backend returned HTTP {resp.status_code} for {target}")
            return None
        return resp.json()
    except Exception as e:
        print(f"[Commodity Proxy Asia] ME backend fetch error for {target}: {e}")
        return None
...
def get_commodity_data(target, force=False):
    """
    Three-layer cascade:
      1. Asia Redis cache fresh? → return it.
...
PROACTIVE_REFRESH_TARGETS = ['china', 'japan', 'taiwan', 'vietnam', 'afghanistan']
        ...
        try:
            for target in PROACTIVE_REFRESH_TARGETS:
                ...
                if age > COMMODITY_CACHE_TTL_SECONDS:
                    print(f"[Commodity Proxy Asia] Background refresh: {target}")
                    fresh = _fetch_from_me_backend(target)
                    if fresh:
                        _save_to_redis(target, fresh)
                        with _refresh_lock:
            ...
            print(f"[Commodity Proxy Asia] Background loop error: {e}")
...
def _start_background_worker():
    ...
    t.start()
    print("[Commodity Proxy Asia] ✅ Background refresh worker started (12h cadence)")
...
def register_commodity_proxy(app, start_background=True):
    ...
    @app.route('/api/asia/commodity/<target>', methods=['GET', 'OPTIONS'])
    def api_asia_commodity_target(target):
    ...
    @app.route('/api/asia/commodity-debug', methods=['GET'])
    def api_asia_commodity_debug():
    ...
    # passthroughs let Asia country-stability pages call their own regional
    # backend without needing to know where the data lives. No local caching —
    # ME fingerprint already has 12h TTL.

    @app.route('/api/asia/leader-interventions/<country>', methods=['GET', 'OPTIONS'])
    def api_asia_leader_interventions_country(country):
    ...
    @app.route('/api/asia/leader-interventions/commodity/<commodity>', methods=['GET', 'OPTIONS'])
    def api_asia_leader_interventions_commodity(commodity):
    ...
    @app.route('/api/asia/leader-interventions', methods=['GET', 'OPTIONS'])
    def api_asia_leader_interventions_global():
    ...
    # absorption_signatures.py. These passthroughs let Asia country pages call
    # their own regional backend.

    @app.route('/api/asia/absorption-signature/<intervention_id>', methods=['GET', 'OPTIONS'])
    def api_asia_absorption_signature(intervention_id):
    ...
    @app.route('/api/asia/absorption-signatures', methods=['GET', 'OPTIONS'])
    def api_asia_absorption_signatures_list():
    ...
    print("[Commodity Proxy Asia] ✅ Endpoints registered:")
    print("  GET  /api/asia/commodity/<target>")
    print("  GET  /api/asia/commodity-debug")
    print("  GET  /api/asia/leader-interventions")
    print("  GET  /api/asia/leader-interventions/<country>")
    print("  GET  /api/asia/leader-interventions/commodity/<commodity>")
    print("  GET  /api/asia/absorption-signature/<intervention_id>")
    print("  GET  /api/asia/absorption-signatures")
